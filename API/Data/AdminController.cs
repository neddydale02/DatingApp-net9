...
public class AdminController(UserManager<AppUser> userManager) : BaseApiController
{
    ...
    public async Task<ActionResult> EditRoles(string username, string roles){
        ...
        var results = await userManager.AddToRolesAsync(user, selectedRoles.Except(userRole));

        if(!results.Succeeded) return BadRequest("Failed to add to roles");

        results =await userManager.RemoveFromRolesAsync(user, userRole.Except(selectedRoles));
